...
## Internal function for seperating metadata from datasets
...
metaSep <- function(inDat, 
		meta = c('label', 'metaGroupId', 'groupId', 'goodPeakCount', 'medMz', 'medRt', 
				'maxQuality', 'note', 'compound', 'compoundId', 'expectedRtDiff', 
				'ppmDiff', 'parent', 'adductName', 'isotopeLabel', 'formula', 'ctName', 'pVal'), excludeName){
...
## pca        - Logical - TRUE makes a create pca scores and returns plot

## returnPCA - logical - TRUE returnes PCA scores
## returnMeta - Logical, if T, will return all input meta data

## jitter - Logical, if true will offset the x scale
...
fuguPlot <- function(inDat,by = "\\.",excludeName = NULL, na.rm  = T,rmZero = F,rCst = F, cCst = F,
	...
	scale <- if (missing(scale)) "none" else match.arg(scale)
	...
	if(vioPlot){
		...
		vio_dat$Group <- sapply(strsplit(vio_dat$Group, split = by), function (x) x[1])
		...
		if (saveFig) {
			ggsave(paste('Violin_Compound',numCmp,'.pdf',sep = "_"),
			plt,
	...
	if(BoxWhisker){
		...
		if (saveFig) {
			ggsave(paste('BoxWhisker_Compound',numCmp,'.pdf',sep = "_"),
			plt,
			height = 8.5,width=11,units = "in")
		}
		}
	...
	if(heatMap){
		...
		metImage( as.matrix(inDat[nrow(inDat):1,]), col = lDat$col, breaks = lDat$breaks, rCst = rCst, cCst = cCst, grid = grid )		
...
## 					'zScore' normalizes relative to a control, sets equal variance
##          'none' does not apply any normalization
## durbScale - integer - constant for durbin trasform (0 is ln)
## excludeName - character, names of columns to be excluded from analysis
## na.rm - Logical - TRUE converts NA to 0, INF to positive max, -INF to negative max
...
levelFun <- function(x, n = 10, r = c(0, 1), g = c(0,0), b = c(1,0), zlim, logScale = T, returnInterval = FALSE){
...
metImage <- function( x, col, breaks, cCst = FALSE, rCst = FALSE, grid = TRUE ){
	...
	#######################################	
	quartz()
	heatmap(as.matrix(x), Colv = Colv, Rowv = Rowv,
	 na.rm = T, col = col, breaks = breaks, grid = grid )	
}
